...
!        injectedflux = nrm*(Em(i)*mbeam/Ecut)**(30)*padist(j)
      ...
      injectedf(i,j) = injectedflux / (clight*btam(i)) * (E(i+1)-E(i))*mbeam*2*pi*(mum(j)-mum(j+1))/msvol(i,j)
    enddo; enddo
    nrm = 0
    do j=1, nmu; do i=1, nE
...
!      f(0,:,:) = 0 ; f(-1,:,:) =0
    ...
    if (reflecttop.and.par_leftedge(3)) then
      ...
      enddo;enddo
      f(-1,1:nmu,-1:0) = f(1,1:nmu,-1:0); f(0,1:nmu,-1:0) = f(1,1:nmu,-1:0)
      f(nE+1,1:nmu,-1:0) = f(nE,1:nmu,-1:0); f(nE+2,1:nmu,-1:0) = f(nE,1:nmu,-1:0)
      f(:,0,-1:0) = f(:,1,-1:0); f(:,nmu+1,-1:0) = f(:,nmu,-1:0)
    ...
    if (reflectbottom.and.par_rightedge(3)) then
      ...
      enddo;enddo
      f(0,1:nmu,nz+1:nz+2) = f(1,1:nmu,nz+1:nz+2); f(nE+1,1:nmu,nz+1:nz+2) = f(nE,1:nmu,nz+1:nz+2)
      f(:,0,nz+1:nz+2) = f(:,1,nz+1:nz+2); f(:,nmu+1,nz+1:nz+2) = f(:,nmu,nz+1:nz+2)
    else if (par_rightedge(3)) then
      f(:,:,nz+1) = f(:,:,nz)
      f(:,:,nz+2) = f(:,:,nz) ! or should this also be set to 0 ?
    endif
    call par_updateGC(f)
  ENDSUBROUTINE
